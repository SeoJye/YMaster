...
<%
	...
	String search = request.getParameter("searchConditionKind");

	
	if(search.equals("k1")) search = "치킨";
	else if(search.equals("k2")) search = "피자";
	else if(search.equals("k3")) search = "떡볶이";
	else if(search.equals("k4")) search = "돈까스,일식";
	else if(search.equals("k5")) search = "중식";
	else if(search.equals("k6")) search = "한식";
	else if(search.equals("k7")) search = "기타";

	System.out.println(search);
	
	List<DeliveryVO> dList = dao.getDeliveryListSearchKind(search);
	if(search.equals("k0")) response.sendRedirect("delivery.jsp");
...
<body>
	...
	<div align="center">
		...
		<form action="deliverySearch.jsp" method="post">
			<table border="1" cellpadding="0" width="850">
				<tr>
					<td>종류 검색</td>
					<td align="right">
						<select name="searchConditionKind">
							...
							<% } %>
							<% if(search.equals("한식")) { %>
							<option selected value="k6">한식</option>
							<% } else { %>
							<option value="k6">한식</option>
							<% } %>
							<% if(search.equals("기타")) { %>
							<option selected value="k7">기타</option>
							<% } else { %>
		...
		<input type="button" style="background-color:#ccccff; width:30%; height:20pt;" value="메인으로" onclick="location.href='main.jsp'"/>
	</div>
</body>
</html>